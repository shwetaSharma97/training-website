  ...
  return (
    <div className="flex flex-col items-center max-w-[300px] text-center p-4">
      ...
      <div className="w-14 h-14 flex items-center justify-center rounded-full bg-blue-100/50 mb-4 border border-blue-200/50 shadow-md">
        ...
        {/* Increased stroke for better visibility */}
      </div>
      ...
      <p className="text-white text-sm leading-normal">{description}</p>
    </div>
  );
};
...
const PrimaryButton = ({ children }: { children: ReactNode }) => (
  <Link href="#enroll">
    <button className="px-5 py-4 text-md font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-transform transform hover:scale-[1.02] duration-300 shadow-xl shadow-blue-400/50">
      {children}
    </button>
  </Link>
);
...
export default function HeroSection() {
  return (
    ...
    >
      ...
      ></div>
      <div
        className="absolute top-1/4 left-[-20%] w-32 h-32 md:w-48 md:h-48 opacity-60 md:opacity-80 z-0 bg-gradient-to-r from-blue-300 to-orange-500"
        style={{
          WebkitMaskImage:
            "radial-gradient(circle, black 2px, transparent 2px)",
          WebkitMaskRepeat: "repeat",
      ...
      >
        <motion.div
          ...
          className="inline-flex items-center mt-5 space-x-2 px-4 py-2 bg-orange-50 text-orange-600 rounded-full text-sm font-semibold shadow-sm border border-orange-200"
        >
        ...
        <motion.p
          variants={itemVariants}
          className="mt-6 text-base md:text-xl text-white max-w-3xl mx-auto"
        >
          45 days of intensive training • Guaranteed internship • Full-time job
          placement
        </motion.p>
        ...
        <motion.div
          variants={itemVariants}
          className="grid grid-cols-2 md:grid-cols-4 place-items-center gap-y-8 mt-16 md:mt-20"
        >
          <FeatureBadge
            icon={Code}
